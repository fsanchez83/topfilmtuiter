...
def get_films(usuario, nombre):
    ...
    for i in lista_pelis:
        ...
        anio = titulo_anio[-4:]
        lista_films.append([posicion, titulo, anio,url])
...
if __name__ == '__main__':
    ...
    for nombre_usr in usuarios:
        lista_films,url = get_films(nombre_usr, nombre_lista)
        if len(lista_films) > 0:
            contador = contador + 1
            print(url)
        else:
            lista_films, url = get_films(nombre_usr, "topfilmtuiter")
            if len(lista_films) > 0:
                contador = contador + 1
                print(url)
            else:
                lista_films, url = get_films(nombre_usr, "top-filmtuitter")
                if len(lista_films) > 0:
                    contador = contador + 1
                    print(url)
                else:
                    lista_films, url = get_films(nombre_usr, "top-filmtuiter-1")
                    if len(lista_films) > 0:
                        contador = contador + 1
                        print(url)
                    else:
                        lista_films, url = get_films(nombre_usr, "top-filmtwitter")
                        if len(lista_films) > 0:
                            contador = contador + 1
                            print(url)
                        else:
                            lista_films, url = get_films(nombre_usr, "top-filmtuiter-4")
                            if len(lista_films) > 0:
                                contador = contador + 1
                                print(url)
                            else:
                                lista_films, url = get_films(nombre_usr, "top-fimtuiter")
                                if len(lista_films) > 0:
                                    contador = contador + 1
                                    print(url)
                                else:
                                    sin_lista.append(nombre_usr)
    ...
    df_lista = pd.DataFrame(flat_list, columns=['Pos', 'Titulo', 'Anio','url'])
    ...
    resultado_final = df_lista.groupby(['Titulo', 'Anio']).agg({'Puntos': 'sum', 'Titulo': 'count'}).rename(
        columns={"Puntos": "Puntos", "Titulo": "Menciones"}).sort_values(
        ['Puntos', 'Menciones'], ascending=[False, False])

    resultado_final_lb = resultado_final
    ...
    resultado_final_lb = resultado_final_lb.drop(['Puntos', 'Menciones'], axis=1)
    resultado_final_lb.to_csv('Lista_votaciones_stats_lb.csv')
